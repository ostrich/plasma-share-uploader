...
bool validateMimePattern(const QString& messagePath, const QString& jsonPath, const QString& pattern,
    const QString& codePrefix, QList<TargetDiagnostic>* diagnostics)
{
    if (!TargetFormat::mimePattern(pattern))
        return appendDiagnostic(diagnostics, jsonPath, codePrefix + QStringLiteral(".invalid"),
            messagePath + QStringLiteral(" must contain exact MIME types, type/*, or */*"));
    return true;
}
}

bool TargetPreUploadConfigParser::parse(
    const QJsonObject& target, ParsedPreUploadConfig* parsed, QList<TargetDiagnostic>* diagnostics)
{
    ...
    if (!value.isArray()) {
        return appendDiagnostic(diagnostics, QStringLiteral("/preUpload"), QStringLiteral("preUpload.type"),
    ...
    for (int i = 0; i < rules.size(); ++i) {
        if (!rules.at(i).isObject()) {
            ok = appendDiagnostic(diagnostics, QStringLiteral("/preUpload/%1").arg(i),
        ...
        if (mimePatterns.isEmpty()) {
            ok = appendDiagnostic(diagnostics, QStringLiteral("%1/mime").arg(ruleJsonPath),
            ...
            for (int mimeIndex = 0; mimeIndex < mimePatterns.size(); ++mimeIndex) {
                ...
                if (!patternValue.isString()) {
                    ok = appendDiagnostic(diagnostics, QStringLiteral("%1/mime/%2").arg(ruleJsonPath).arg(mimeIndex),
                ...
                ok = validateMimePattern(QStringLiteral("%1.mime").arg(rulePath),
                         QStringLiteral("%1/mime/%2").arg(ruleJsonPath).arg(mimeIndex), pattern,
                         QStringLiteral("preUpload.mime"), diagnostics)
                    && ok;
                if (!pattern.isEmpty()) {
                    parsedRule.mimePatterns.append(pattern.toLower());
                }
            }
        }

        const QString fileHandling = stringValue(rule, "fileHandling");
        const bool validFileHandling
            = fileHandling == QLatin1StringView("inplace_copy") || fileHandling == QLatin1StringView("output_file");
        if (!validFileHandling) {
            ok = appendDiagnostic(diagnostics, QStringLiteral("%1/fileHandling").arg(ruleJsonPath),
        ...
        if (commands.isEmpty()) {
            ok = appendDiagnostic(diagnostics, QStringLiteral("%1/commands").arg(ruleJsonPath),
        ...
        for (int commandIndex = 0; commandIndex < commands.size(); ++commandIndex) {
            if (!commands.at(commandIndex).isObject()) {
                ok = appendDiagnostic(diagnostics, QStringLiteral("%1/commands/%2").arg(ruleJsonPath).arg(commandIndex),
            ...
            if (argv.isEmpty()) {
                ok = appendDiagnostic(diagnostics, QStringLiteral("%1/argv").arg(commandJsonPath),
            ...
            for (int argIndex = 0; argIndex < argv.size(); ++argIndex) {
                if (!argv.at(argIndex).isString() || argv.at(argIndex).toString().isEmpty()) {
                    ok = appendDiagnostic(diagnostics, QStringLiteral("%1/argv/%2").arg(commandJsonPath).arg(argIndex),
                ...
                while (it.hasNext()) {
                    ...
                    if (placeholder != QLatin1StringView("FILE") && placeholder != QLatin1StringView("OUT_FILE")) {
                        ok = appendDiagnostic(diagnostics,
                            ...
                            QStringLiteral("%1.argv contains unsupported placeholder ${%2}")
                                .arg(commandPath, placeholder));
                    }
                }
            }
        ...
        if (validFileHandling && fileHandling == QLatin1StringView("output_file") && !sawOutFile) {
            ok = appendDiagnostic(diagnostics, QStringLiteral("%1/commands/0/argv").arg(ruleJsonPath),
        ...
        if (!timeout.isUndefined()) {
            if (!timeout.isDouble() || timeout.toDouble() != timeout.toInt() || timeout.toInt() <= 0) {
                ok = appendDiagnostic(diagnostics, QStringLiteral("%1/timeoutMs").arg(ruleJsonPath),
