...
void openWallet(QObject *context, std::function<void(KWallet::Wallet *, QString)> callback)
{
    ...
    if (!wallet) {
        QTimer::singleShot(0, context, [callback]() { callback(nullptr, QStringLiteral("KWallet is unavailable or access was cancelled.")); });
        return;
    }
    wallet->setParent(context);
    QObject::connect(wallet, &KWallet::Wallet::walletOpened, context, [wallet, callback](bool ok) {
        const bool ready = ok && (wallet->hasFolder(folder) || wallet->createFolder(folder)) && wallet->setFolder(folder);
        callback(ready ? wallet : nullptr, ready ? QString{} : QStringLiteral("Could not open the uploader's KWallet folder."));
        wallet->deleteLater();
    });
}
...
{
    if (value.isString()) out.append(value.toString());
    else if (value.isArray()) for (const auto &item : value.toArray()) strings(item, out);
    else if (value.isObject()) {
        const auto object = value.toObject();
        for (auto it = object.begin(); it != object.end(); ++it) strings(it.value(), out);
    }
}
}
...
{
    return QRegularExpression(QStringLiteral("^[A-Za-z0-9_.-]+$")).match(key).hasMatch();
}
...
void CredentialStore::read(const QStringList &keys, QObject *context, ReadCallback callback)
{
    ...
    openWallet(context, [keys, callback](KWallet::Wallet *wallet, const QString &error) {
        if (!wallet) { callback({}, error); return; }
        Values values;
        for (const auto &key : keys) {
            QString value;
            if (!validKey(key) || !wallet->hasEntry(key) || wallet->readPassword(key, value) != 0 || value.isEmpty()) {
                callback({}, QStringLiteral("Credential '%1' is missing or empty. Configure it in the target manager.").arg(key));
                return;
            }
...
void CredentialStore::write(const QString &key, const QString &value, QObject *context, WriteCallback callback)
{
    if (!validKey(key) || value.isEmpty()) {
        QTimer::singleShot(0, context, [callback]() { callback(QStringLiteral("Enter a credential name (letters, digits, dot, dash, underscore) and a nonempty secret.")); });
        return;
    }
    openWallet(context, [key, value, callback](KWallet::Wallet *wallet, const QString &error) {
        callback(!wallet ? error : wallet->writePassword(key, value) == 0 ? QString{} : QStringLiteral("Could not save credential to KWallet."));
    });
}
...
void CredentialStore::remove(const QString &key, QObject *context, WriteCallback callback)
{
    openWallet(context, [key, callback](KWallet::Wallet *wallet, const QString &error) {
        callback(!wallet ? error : wallet->removeEntry(key) == 0 ? QString{} : QStringLiteral("Could not remove credential from KWallet."));
    });
}
...
QStringList CredentialStore::requestStrings(const ParsedRequestConfig &request)
{
    ...
    switch (request.type) {
    case RequestBodyType::Multipart: out.append(request.multipartFields.values()); break;
    case RequestBodyType::FormUrlencoded: out.append(request.formFields.values()); break;
    case RequestBodyType::Json: strings(request.jsonFields, out); break;
    case RequestBodyType::Raw: break;
    }
    return out;
}
...
QStringList CredentialStore::walletKeys(const ParsedRequestConfig &request)
{
    ...
    for (const auto &text : requestStrings(request)) {
        ...
        while (matches.hasNext()) keys.append(matches.next().captured(1));
    }
...
QStringList CredentialStore::missingEnvironment(const ParsedRequestConfig &request)
{
    ...
    for (const auto &text : requestStrings(request)) {
        ...
        while (matches.hasNext()) {
            ...
            if (qgetenv(key.toUtf8().constData()).isEmpty()) missing.append(key);
        }
    }
...
QStringList CredentialStore::secretValues(const ParsedRequestConfig &request, const Values &walletValues)
{
    ...
    for (const auto &text : requestStrings(request)) {
        auto matches = pattern.globalMatch(text);
        while (matches.hasNext()) values.append(QString::fromLocal8Bit(qgetenv(matches.next().captured(1).toUtf8().constData())));
    }
    ...
    auto addSecret = [&values](const QString &value) {
        values.append(value);
        if (value.startsWith(QLatin1StringView("Bearer "), Qt::CaseInsensitive) || value.startsWith(QLatin1StringView("Basic "), Qt::CaseInsensitive))
    ...
    for (const auto &map : {request.headers, request.query, request.multipartFields, request.formFields})
        for (auto it = map.begin(); it != map.end(); ++it) if (sensitive.match(it.key()).hasMatch()) addSecret(it.value());
    std::function<void(QJsonValue)> collect = [&](QJsonValue value) {
        if (value.isArray()) for (const auto &v : value.toArray()) collect(v);
        if (value.isObject()) {
            const auto object = value.toObject();
            for (auto it = object.begin(); it != object.end(); ++it) {
                if (sensitive.match(it.key()).hasMatch() && it.value().isString()) addSecret(it.value().toString());
                else collect(it.value());
            }
        }
    };
    collect(request.jsonFields);
    const QUrl url(request.url);
    if (!url.password().isEmpty()) addSecret(url.password());
    for (const auto &item : QUrlQuery(url).queryItems(QUrl::FullyDecoded)) if (sensitive.match(item.first).hasMatch()) addSecret(item.second);
...
QString CredentialStore::redact(QString text, const QStringList &secrets)
{
    ...
    for (const auto &value : variants) if (!value.isEmpty()) text.replace(value, QStringLiteral("[redacted]"));
    return text;
}
