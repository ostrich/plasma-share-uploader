...
void applyHeaders(const QMap<QString, QString>& headers, const QFileInfo& fileInfo, QNetworkRequest& requestObj,
    const QMap<QString, QString>& secrets = { });
QString substituteEnv(const QString& value);
QString substituteRequestValue(
    const QString& value, const QFileInfo& fileInfo, const QMap<QString, QString>& secrets = { });
QString applyUrlTemplate(
...
QUrl applyQueryParameters(const QString& urlTemplate, const QJsonObject& requestConfig, const QFileInfo& fileInfo,
    const QMap<QString, QString>& secrets = { });
QUrl applyQueryParameters(const QString& urlTemplate, const QMap<QString, QString>& queryItems,
    const QFileInfo& fileInfo, const QMap<QString, QString>& secrets = { });
QByteArray createFormUrlencodedBody(
    const QMap<QString, QString>& fields, const QFileInfo& fileInfo, const QMap<QString, QString>& secrets = { });
QJsonValue substituteJsonValue(
...
bool isValidXmlPath(const QString& path);
QJsonValue resolveJsonPointer(const QJsonValue& root, const QString& pointer);
QString resolveXmlPath(const QByteArray& xmlBytes, const QString& path);
}
